...
`NODE_ENV`, `HOST` and `PORT` come from the parent environment. The integrated service calls Center's configured read-only RPC gateway, store and pinning service directly through bounded callbacks, and dispatches the two intent write routes to Center's own Hono app in process on an internal marker carried in the app's environment. There is no network hop and no impersonated approved browser Origin. RPC retains the owning gateway's provider failover, read-method allowlist and credential sanitization. Providers must support the canonical block-hash reads used by the MCP. Standalone `RPC_URL_*` and `JBCENTER_ORIGIN` settings are not host integration settings.
