...
The parent `src/server.ts` dispatches `/mcp` and its subpaths to the raw MCP HTTP handler before Center's browser API middleware. Parent `src/mcp.ts` injects bounded callbacks for Center's reads, allowlisted read-only RPC, and reviewed JSON pinning. These callbacks preserve cancellation, response limits, provider error sanitization and PostgreSQL-backed shared quotas. Reads call the owning store/gateway/pinning objects directly. The two intent write routes — publishing a signed intent and requesting a sponsored deploy — are dispatched to Center's own Hono app in process, marked internal through the app's environment rather than a header, so they reuse Center's signature verification, publish limits and sponsor policy. Neither path makes a network hop or fabricates an approved Origin. Standalone HTTP and stdio retain their ordinary external read adapters and have no pinning callback by default.
