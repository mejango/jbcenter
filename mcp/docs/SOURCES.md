...
| `deploy-all-v6`                                                        | README, architecture, risks, administration and deployment runbook, including per-chain execution and migration status                                                                                                                                       |
...
| `nana-buyback-hook-v6`, `nana-router-terminal-v6`                      | All buyback, registry, router, gateway and supporting source, plus README, architecture, risks and administration guidance                                                                                                                                   |
...
| `bendystraw-v6`                                                        | README, actual Ponder schema, API routing, ID construction, gateway event handling and the executed rollout manifest                                                                                                                                         |
...
The source workspace is the EVM workspace containing `extensions/jbcenter/mcp`. The current skills checkout defaults to `skills/` inside that workspace. Configure both explicitly when the checkout layout differs:
...
`npm run rollout:sync -- --workspace /path/to/evm` refreshes `data/rollout.json` from the flat `deploy-all-v6/deployments/<chain>/` records. `npm run rollout:check` verifies the same source bytes, hashes and Git provenance. Runtime reads this committed bundle without consulting sibling source trees. Canonical, `_deprecated1` (previous) and `_deprecated` (v1) addresses remain represented.

Only a chain with an executed gateway or ratio-feed record exposes that address. The rollout sync rejects missing, failed or unmined receipts. A reviewed mainnet proposal does not change its active deployment record. After execution and distribution, regenerate the rollout, knowledge and development bundles and review the diff. Gateway reads resolve `terminalOf(projectId)` and then the selected gateway's immutable `ROUTER`; the outer terminal remains the approval and transaction destination.

The gateway ABI includes pending commitments/failures, retry/finalize calls and their events. A queued fee remains in custody until settled or refunded; a refund does not restore core `feeFreeSurplusOf`. `pendingCallCount` is cumulative allocated IDs, not an outstanding-balance count. Retained amounts and source projects come from authenticated queue events.
