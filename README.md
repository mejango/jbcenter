...
- `WALLET_FRAMEABLE_APP_ORIGINS` — optional comma-separated app origins admitted to frame their own payment reviews and sign-ins inside their pages (`frame-ancestors` names the one app each review or intent was prepared for). Inside a frame no Center cookie takes part: a review is admitted by its id, a sign-in by its intent id plus the launch signature kept on the row, and one passkey assertion naming the app as its top origin approves. Signing up and recovery stay top-level. Absent, nothing frames Center.
