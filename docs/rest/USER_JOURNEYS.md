...
| Automate protected reads or plan preparation | [Accounts](/accounts) → Sign in with an existing wallet → API access → approve and download a connection → bot signs requests. | The owner approves account enrollment and the exact bot grant. The bot's API permissions (its **grant**) have enforced scopes and expiry. | Implemented. Bot reads and plans need no recurring wallet prompts; a grant gives no onchain spending authority. |
