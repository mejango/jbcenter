...
| Deployment approval and transaction codec (part of W4) | A separate fresh deployment document binds the verified enrollment and exact factory call. The pure validator rejects changed EIP1559 fields, noncanonical raw encoding, and transactions outside the relay policy. Recovery validation preserves the original bytes after approval expiry. | This does not reserve treasury allocation or a sender nonce, sign, broadcast or prove canonical deployment. The pilot codec requires a positive priority fee. Its execution-cost bound excludes Base L1 and operator charges. See the [deployment strategy](WALLET-DEPLOYMENT-STRATEGY.md). |
...
Adapt Beep's prepaid direct-deployment relay behavior to Center's shared PostgreSQL pool and nonce journal. A separate fresh deployment approval must reference the durable verified enrollment. Preserve exact signed bytes before dispatch and reconcile unknown outcomes. Base execution-gas estimates are not the complete native fee liability; prepaid allocation and treasury accounting need their own durable policy. The local EntryPoint bootstrap experiment is not a hosted-bundler admission claim.
