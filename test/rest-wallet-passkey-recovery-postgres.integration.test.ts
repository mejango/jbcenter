...
suite('durable replacement-passkey proof intake (canonical state explicitly modeled)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const name of ['004_rest_accounts.sql', '007_rest_smart_accounts.sql', '012_rest_smart_account_onboarding.sql',
      ...
      '017_rest_wallet_policy.sql', '051_wallet_policy_app_grant_lifetime.sql', '019_rest_wallet_app_grants.sql', '052_wallet_app_grant_lifetime_90d.sql', '020_rest_wallet_authority.sql', '039_wallet_authority_window.sql', '049_wallet_authority_window_15m.sql', '028_wallet_recovery.sql', '029_wallet_recovery_mapping.sql', '033_wallet_unproved_recovery_expiry.sql', '030_wallet_recovery_flow.sql'])
