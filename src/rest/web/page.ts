...
import { paraDialogHtml, paraCss } from "./paraPage.js";
import { smartWalletSections } from "./smartPage.js";

function escapeHtml(value: string): string {
  return value.replace(/[&<>"']/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[character]!);
}
export function accountsPage(options: { scriptPath?: string; stylePath?: string; audience?: string; para?: { apiKey: string; environment: "BETA" | "PROD" } } = {}): string {
  return `<!doctype html>
...
<body data-audience="${escapeHtml(options.audience ?? "")}" data-para-api-key="${escapeHtml(options.para?.apiKey ?? "")}" data-para-environment="${escapeHtml(options.para?.environment ?? "BETA")}"><main>
...
<p>Use your email, phone, social account, or an existing wallet.</p>
