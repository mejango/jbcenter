...
<p>Create or connect a supported smart wallet, then review and sign each V6 transaction. No bot or session is required. Each chain has its own wallet.</p>
<p class="service-links"><a href="/api">API guide</a><a href="/api/v1/capabilities">Hosted capabilities</a><a href="/api/docs/smart-accounts">Smart wallet guide</a></p>
<p id="smart-readiness">Load your account to check supported chains automatically.</p>
<details><summary>Hosted configuration</summary><button id="smart-discover" type="button">Refresh supported chains</button><pre id="smart-capabilities" tabindex="0" aria-label="Hosted capability details" hidden></pre></details>
<fieldset id="smart-fields" disabled>
<label>Chain and wallet deployment<select id="smart-manifest"><option value="">Load your account first</option></select></label>
<button id="smart-switch" type="button">Switch wallet to this chain</button>
<details><summary>Create a smart wallet</summary>
<p>Your connected wallet is the default owner and pays the creation fee. Review the address and owners before approving creation. Creating a wallet does not grant a bot permission to use it.</p>
...
<p>For a threshold above one, share the exact public signing document with the other owners and collect their EIP712 signatures. Keep this page open while they sign.</p></details>
...
<section aria-labelledby="operation-heading"><h2 id="operation-heading">05 / Review and execute</h2>
<p>Prepare a V6 action, review the simulated transaction, then sign and submit it. Check its status to confirm it completed.</p>
<fieldset id="operation-fields" disabled>
<label id="operation-authority-label" hidden>Signing authority<select id="operation-authority" disabled><option value="owner">Fresh owner approval</option><option value="session">Active session key</option></select></label>
...
<p>ERC20 payments require a finite terminal allowance approved separately by the owner.</p></details>
...
<p>For verified payment or cash-out results, choose one plan step at a time. A multi-call batch can confirm execution while leaving financial results unverified.</p>
<details><summary>Choose specific plan steps</summary><p>All planned calls are selected by default.</p><label>Steps to execute, zero-based comma-separated indexes<input id="operation-steps" value="0" inputmode="numeric"></label></details>
...
<details id="operation-owner-signatures-label" hidden><summary>Additional owner signatures</summary><label>SafeOp signatures, JSON array<textarea id="operation-owner-signatures" rows="2" spellcheck="false" placeholder='["0x…"]'></textarea></label></details>
...
<p>Optional: authorize a registered bot to repeat a specific action for 7 or 30 days. Fund movement still needs fresh owner approval unless you explicitly approve the exact payment budget below. <a href="/api/docs/sessions">Bot permission guide</a>.</p>
