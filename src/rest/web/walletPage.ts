...
button.secondary{background:transparent;color:inherit}button:disabled{cursor:wait}button.link:disabled,button.secondary:disabled{opacity:.55}button:disabled:not(.link):not(.secondary){border-color:transparent;background:linear-gradient(#172019,#172019) padding-box,repeating-linear-gradient(-45deg,#172019 0 5px,#9aa79c 5px 10px) border-box;background-size:auto,14.14px 14.14px;animation:march 1.4s linear infinite}@keyframes march{to{background-position:0 0,14.14px 0}}@media(prefers-reduced-motion:reduce){button:disabled:not(.link):not(.secondary){animation:none}}button:hover:not(:disabled){background:#364532;color:white}button.secondary:hover:not(:disabled){background:#e8ece4;color:inherit}
...
.qr{width:min(100%,16rem);margin:1rem 0}.qr svg{width:100%;height:auto;display:block}#wallet-device h2{font-size:1.1rem;margin:1.5rem 0 .5rem}@media(max-width:40rem){main{padding-left:2.9rem}#wallet-status::before{left:-1.25em}}@media(max-width:400px){main{padding:1.1rem 1.1rem 1.1rem 2.7rem}.actions{flex-direction:column;align-items:stretch}.actions button{width:100%}.actions button.link{width:auto;align-self:flex-start}}`;
}
