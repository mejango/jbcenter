...
<p id="wallet-open-row" hidden><a id="wallet-open" href="${base || '/'}">Open as a page</a></p>
<noscript><p>Enable JavaScript to sign in with your passkey.</p></noscript>
</main></body></html>`;
}

export function walletCss(): string {
  return `:root{--wallet-font:ui-monospace,SFMono-Regular,Consolas,monospace;--wallet-bg:#f5f4ee;--wallet-fg:#172019;--wallet-muted:#4b5a4e;--wallet-line:#172019;--wallet-accent:#172019;--wallet-accent-fg:#fff;--wallet-radius:0;color-scheme:light;font-family:var(--wallet-font);color:var(--wallet-fg);background:var(--wallet-bg)}
*{box-sizing:border-box}body{margin:0}main{width:min(100%,38rem);margin:clamp(1rem,10vh,6rem) auto;padding:1.5rem}
html.framed main{margin:0 auto;padding:1.25rem calc(1.6rem - 1px) 1.5rem}html.framed .brand{display:none}html.framed h1{margin-top:.5rem}html.framed #wallet-status{margin:1rem 0;padding-left:1.5em}html.framed #wallet-status::before{left:0}
...
p{line-height:1.6;overflow-wrap:anywhere}#wallet-status{color:var(--wallet-fg);font-weight:700;min-height:3rem;margin:1.75rem 0;font-size:.95rem;position:relative}#wallet-status::before{content:"⚡\\FE0E";position:absolute;left:-1.05em;top:0;font-size:1.4em;line-height:1.143em;text-align:center;-webkit-text-stroke:.045em currentColor}#wallet-status[data-state=error]::before,#wallet-status[data-state=retry]::before{content:"!"}#wallet-status:empty::before{content:none}
...
:focus-visible{outline:3px solid #5275d1;outline-offset:4px}.note{font-size:.8rem;color:var(--wallet-muted);margin-top:2rem}#wallet-links{display:flex;flex-wrap:wrap;gap:.75rem 1.5rem;margin-top:1.75rem}#wallet-links a,#wallet-open-row a{color:var(--wallet-muted);font-size:.9rem;text-underline-offset:.15em}#wallet-links a:hover,#wallet-open-row a:hover{color:var(--wallet-fg)}#wallet-open-row{margin:1rem 0 0}
