...
export const baseWalletRecoveryFees = Object.freeze({ gas: 3_000_000n, maxPriorityFeePerGas: 1_000_000n, maximumFeePerGas: 1_000_000_000n });
...
/** Hosted Base recovery relay: the same single Dwellir endpoint and pinned fee runtimes as
 * creation, complete receipt fees per transaction, and lane release only after finality. */
export function createBaseWalletRecovery(options: BaseWalletDeploymentOptions & {
  ...
  return createWalletRecoveryRelay({ pool: options.pool, signer: options.signer, manifest: options.manifest, utility: options.utility,
