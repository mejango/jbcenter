...
/// @notice A SmartSession IUserOpPolicy for the pinned current Pimlico V7 gas-only profile.
...
contract CenterSessionGuardV2 {
    ...
    address public constant PAYMASTER = 0x777777777777AeC03fd955926DbF81597e66834C;
    bytes32 public constant PAYMASTER_CODE_HASH =
    ...
    function initializeWithMultiplexer(address account, bytes32 id, bytes calldata data) external {
        ...
        if (
            config.paymaster != PAYMASTER || config.paymasterCodeHash != PAYMASTER_CODE_HASH
                || config.paymaster.code.length == 0
    ...
    function checkUserOpPolicy(bytes32 id, PackedUserOperation calldata op) external returns (uint256 validationData) {
        ...
        // Current Pimlico V7: header52 + flags1 + validity12 + signature65.
        // The pinned source decodes mode = flags >> 1; only 0x00/0x01 are gas-only mode 0.
        // The low bit permits all bundlers. Modes 1+ may transfer tokens and are forbidden.
        // Its OpenZeppelin ECDSA.recover(bytes) path accepts 65-byte signatures only.
        if (
            paymasterData.length != MAX_PAYMASTER_DATA_LENGTH || paymasterData.length > config.maxPaymasterDataLength
                || uint8(paymasterData[52]) > 1
        ) return 1;
