...
function currentAuthority(actor: RestActor) {
  ...
  if (!context?.authority)
    throw new RestError(
      401,
      "AUTH_REQUIRED",
      "Dispatch requires an authenticated REST request",
    );
  if (context.signal.aborted)
    throw new RestError(
      408,
      "REQUEST_ABORTED",
      "Dispatch authorization was interrupted",
    );
  const { principal, input } = context.authority;
  const owner = parseAccountId(actor.accountId);
  if (
    principal.account.id !== actor.accountId ||
    principal.principalId !== actor.principalId ||
    principal.account.ownerAddress.toLowerCase() !==
      owner.ownerAddress.toLowerCase() ||
    principal.account.authorityChainId !== owner.authorityChainId ||
    !principal.scopes.includes("relay")
  )
    return mismatch();
  const { claims, signature } = readRequestClaims(input);
  if (
    claims.accountId !== actor.accountId ||
    claims.signer.toLowerCase() !== principal.signer.toLowerCase() ||
    claims.nonce !== principal.requestNonce ||
  ...
  if (principal.isOwner) {
    if (
      ...
      claims.signer.toLowerCase() !== owner.ownerAddress.toLowerCase()
    )
      return mismatch();
  } else if (
    !principal.grantId ||
    actor.principalId !== `bot:${principal.grantId}`
  )
    return mismatch();
...
function requestBody(
  ...
  if (
    authority.claims.method !== "POST" ||
    authority.claims.contentType.split(";")[0]?.trim().toLowerCase() !==
      "application/json" ||
    authority.input.body.byteLength > 1_048_576
  )
    return mismatch();
  try {
    const value: unknown = JSON.parse(
      new TextDecoder("utf-8", { fatal: true }).decode(authority.input.body),
    );
    if (value && typeof value === "object" && !Array.isArray(value))
      return value as Record<string, unknown>;
  } catch {
    /* No request content belongs in an authorization error. */
  }
  return mismatch();
}
...
function transactionIntent(
  ...
  if (
    ...
    `/api/v1/plans/${planId}/steps/${stepIndex}/submissions`
  ) {
    if (
      Object.keys(body).some(
        (key) => !["rawSignedTransaction", "ownerApproval"].includes(key),
      )
    )
      return mismatch();
    raw = body.rawSignedTransaction;
  } else if (
    authority.claims.requestTarget === `/api/v1/plans/${planId}/submissions`
  ) {
    if (
      Object.keys(body).some((key) => key !== "submissions") ||
    ...
    for (const entry of body.submissions) {
      ...
      if (
        Object.keys(item).some(
          (key) =>
            !["stepIndex", "rawSignedTransaction", "ownerApproval"].includes(
              key,
            ),
        ) ||
        !Number.isSafeInteger(item.stepIndex) ||
        Number(item.stepIndex) < 0 ||
        Number(item.stepIndex) > 31 ||
        seen.has(Number(item.stepIndex))
      )
        return mismatch();
  ...
  if (
    ...
    keccak256(raw as Hex).toLowerCase() !== transactionHash.toLowerCase()
  )
    return mismatch();
}

function sponsorshipIntent(
  authority: ReturnType<typeof currentAuthority>,
  ...
  if (
    authority.claims.requestTarget !==
      `/api/v1/sponsorships/${record.id}/submissions` ||
    Object.keys(body).some(
      (key) => !["signatures", "ownerApproval"].includes(key),
    ) ||
  ...
  let actual: Hex;
  try {
    actual = sponsorshipSubmissionHash(
      record.commitment,
      body.signatures as Hex[],
    );
  } catch {
    return mismatch();
  }
...
function fresh(window: DispatchApprovalWindow, now: number): void {
  if (!Number.isSafeInteger(now) || now <= 0)
    throw new RestError(
      ...
      "The approval clock is unavailable",
    );
  if (
    ...
    window.expiresAt - window.issuedAt > 300 ||
    window.expiresAt <= now ||
    window.issuedAt > now + 30
  ) {
    throw new RestError(
      403,
      "OWNER_APPROVAL_EXPIRED",
      "A fresh owner approval is required before dispatch",
    );
  }
}
...
function completedWindow(
  authority: ReturnType<typeof currentAuthority>,
  window: DispatchApprovalWindow,
  now: number,
): DispatchApprovalWindow {
  if (authority.context.signal.aborted)
    throw new RestError(
      408,
      "REQUEST_ABORTED",
      "Dispatch authorization was interrupted",
...
function normalizedOptions(options: DispatchAuthorityOptions) {
  ...
  return {
    ...
    ...(verifier
      ? {
          verifyContractOwner: (async (input) =>
            (await verifier(input)) === true) satisfies ContractOwnerVerifier,
        }
      : {}),
  };
}
...
async function ownerWindow(
  ...
  await verifyRequestSignature(
    ...
    authority.claims,
    authority.signature,
    options.verifyContractOwner,
    authority.context.signal,
  );
  if (authority.context.signal.aborted)
    throw new RestError(
      408,
...
export function createTransactionDispatchAuthorizer(
  inputOptions: DispatchAuthorityOptions,
) {
  ...
  return async (
    ...
    if (authority.principal.isOwner)
      transactionIntent(authority, plan.id, stepIndex, transactionHash);
    const owner = await ownerWindow(authority, options);
    if (owner) return completedWindow(authority, owner, options.now());
    const verified = await verifyTransactionApproval(
      options.audience,
      authority.context.ownerApprovals.get(stepIndex),
      binding,
      {
        now: options.now,
        signal: authority.context.signal,
        ...(options.verifyContractOwner
          ? { verifyContractOwner: options.verifyContractOwner }
          : {}),
      },
    );
...
export function createSponsorshipDispatchAuthorizer(
  inputOptions: DispatchAuthorityOptions,
) {
  const options = normalizedOptions(inputOptions);
  return async (
    ...
    if (authority.principal.isOwner)
      sponsorshipIntent(authority, record, submissionHash);
    const owner = await ownerWindow(authority, options);
    if (owner) return completedWindow(authority, owner, options.now());
    const verified = await verifySponsorshipApproval(
      options.audience,
      authority.context.sponsorshipOwnerApproval,
      binding,
      {
        now: options.now,
        signal: authority.context.signal,
...
/** Request authentication is separate from the owner/session signature on the UserOperation. */
export function createUserOperationRequestAuthorizer(
  inputOptions: DispatchAuthorityOptions,
) {
  const options = normalizedOptions(inputOptions);
  return async (
    ...
    if (
      authority.claims.requestTarget !==
        `/api/v1/user-operations/${id}/submissions` ||
      Object.keys(body).some((key) => key !== "signature") ||
    ...
    return completedWindow(authority, authority.claims, options.now());
  };
}

/** Admits a reviewed owner setup plan. Actual installation still needs a Safe owner signature. */
export function createSessionPlanAuthorizer(
  inputOptions: DispatchAuthorityOptions,
) {
  const options = normalizedOptions(inputOptions);
  return async (
    ...
    if (
      ...
      authority.claims.requestTarget !==
        `/api/v1/smart-accounts/sessions/${id}/${kind}-plans` ||
      Object.keys(body).some((key) => key !== "compiledHash") ||
      body.compiledHash !== compiledHash
    )
      return mismatch();
    const window = await ownerWindow(authority, options);
    if (!window) return mismatch();
    return {
      ...completedWindow(authority, window, options.now()),
      digest: hashTypedData(
        buildRequestTypedData(options.audience, authority.claims),
      ),
    };
  };
}
