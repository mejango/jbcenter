...
export interface TransactionStore {
  create(plan: StoredPlan, idempotency: IdempotencyClaim, now: number): Promise<StoredPlan>;
  findIdempotentPlan(
    actor: RestActor,
  ...
  claimSubmission(claim: SubmissionClaim): Promise<{ plan: StoredPlan; dispatch: boolean }>;
...
export function storageLimit(): never {
  throw new RestError(429, 'STORAGE_LIMIT', 'Transaction storage limit exceeded.');
}
...
export function assertTime(value: number): void {
  if (!Number.isSafeInteger(value) || value < 0) invalid('Invalid transaction timestamp.');
}
...
export function canRead(plan: StoredPlan, actor: RestActor): boolean {
  return (
    owns(plan, actor) ||
    (actor.accountId === plan.actor.accountId && actor.principalId === `owner:${actor.accountId}`)
  );
}
...
export function nonceKey(attempt: SignedAttempt): string {
  return JSON.stringify([attempt.chainId, attempt.sender.toLowerCase(), attempt.nonce]);
}
export function assertIdempotency(value: IdempotencyClaim): void {
  if (!value || typeof value.key !== 'string' || !/^[\x21-\x7e]{1,128}$/.test(value.key))
...
export function checkIdempotency(
  ...
  if (
    ...
    (existing.requestHash !== claim.requestHash ||
      ...
      (target && (existing.planId !== target.planId || existing.stepIndex !== target.stepIndex)))
  )
...
export function assertAttempt(value: SignedAttempt): void {
  ...
  for (const amount of [value.nonce, value.gas, value.maximumFeePerGas, value.maximumCost]) {
    if (
...
export function assertSavedSteps(plan: StoredPlan, steps: StoredStep[]): void {
  ...
  for (const [index, step] of steps.entries()) {
    ...
    if (step.index !== index || !states.includes(step.state)) invalid('Invalid transaction step.');
    if (
      (step.externalExecution || previous.externalExecution) &&
      !isDeepStrictEqual(step, previous)
    )
      conflict('External execution observations require their bound transport update.');
    if (!isDeepStrictEqual(step.attempt, previous.attempt))
      conflict('Signed transaction reservations can only be changed through submission claims.');
  }
...
export function prepareClaim(
  ...
  if (step.externalExecution) conflict('This step is bound to an external execution transport.');
  if (step.attempt && !sameAttempt(step.attempt, claim.attempt))
    conflict('A different signed transaction is already reserved for this step.');
  ...
  if (plan.revision !== claim.expectedRevision)
    conflict('Transaction plan changed; inspect it before retrying.');
  if (plan.expiresAt <= claim.now) conflict('Transaction plan expired before the dispatch claim.');
  ...
  for (const dependency of plan.draft.calls[claim.stepIndex]!.dependsOn) {
    ...
    if (
      ...
      conflict('Transaction dependencies are not confirmed with verified outcomes.');
  }
  if (
...
export function settle(
  ...
  if (!current.attempt || current.attempt.leaseToken !== leaseToken) return plan;
  const next = boundedClone(plan);
  ...
  if (patch.broadcastAt !== undefined) step.attempt!.broadcastAt = patch.broadcastAt;
  if (patch.lastError !== undefined) step.attempt!.lastError = boundedClone(patch.lastError);
...
export function isRecoverable(plan: StoredPlan): boolean {
  return plan.steps.some(
    (step) => (step.attempt || step.externalExecution) && recoverableStates.includes(step.state),
  );
}

export function assertExternalIndexes(plan: StoredPlan, indexes: readonly number[]): void {
  if (
    !Array.isArray(indexes) ||
    indexes.length < 1 ||
    indexes.length > TRANSACTION_STORAGE_LIMITS.maximumSteps ||
    new Set(indexes).size !== indexes.length ||
    indexes.some((index) => !Number.isSafeInteger(index) || index < 0 || index >= plan.steps.length)
  )
...
export function attachExternalExecutions(
  ...
  for (const binding of bindings) {
    if (binding.transport !== 'relayr') continue;
    ...
    if (step.attempt)
      conflict('An owner-signed attempt cannot be replaced by a forwarded execution.');
    if (step.externalExecution) {
      if (
        step.externalExecution.transport !== 'relayr' ||
        step.externalExecution.bindingId !== binding.bindingId ||
        step.externalExecution.chainId !== chainId
      )
        conflict('The external execution binding differs from its permanent reservation.');
      continue;
    }
    if (step.state !== 'waiting' || step.receipt || step.semantic)
      conflict('The unbound step already has execution observations.');
    step.externalExecution = { transport: 'relayr', bindingId: binding.bindingId, chainId };
    step.state = 'reserved';
    changed = true;
  }
  ...
  for (const update of updates) {
    if (
      Object.keys(update).some(
        (key) => !['index', 'state', 'transactionHash', 'receipt', 'semantic'].includes(key),
      ) ||
    ...
    if (
      previous.attempt ||
      !external ||
      external.transport !== 'relayr' ||
      external.bindingId !== bindingId ||
      external.chainId !== plan.draft.calls[update.index]!.chainId
    )
      conflict('The observation does not match its admitted external execution.');
    if (update.transactionHash !== undefined && !/^0x[0-9a-fA-F]{64}$/.test(update.transactionHash))
    ...
    if (update.receipt) {
      ...
      if (
        ...
        receipt.transactionHash.toLowerCase() !== update.transactionHash.toLowerCase() ||
    ...
    if (
      update.semantic &&
      !['verified', 'failed', 'unknown', 'unmodeled'].includes(update.semantic.status)
    )
      invalid('Invalid semantic execution outcome.');
    if (
      update.state === 'confirmed' &&
      (!update.receipt || !update.receipt.canonical || update.receipt.status !== 'success')
    )
      invalid('Confirmed external execution requires a canonical successful receipt.');
    if (
      update.state === 'reverted' &&
      (!update.receipt ||
        !update.receipt.canonical ||
        (update.receipt.status !== 'reverted' && update.semantic?.status !== 'failed'))
    )
      invalid('Reverted external execution requires proof of transaction or inner-call failure.');
    next.steps[update.index] = {
      ...
      externalExecution: {
        ...
        ...(update.transactionHash ? { transactionHash: update.transactionHash } : {}),
      },
...
export function encodeCursor(plan: StoredPlan): string {
  return Buffer.from(JSON.stringify({ createdAt: plan.createdAt, id: plan.id })).toString(
    'base64url',
  );
}
export function decodeCursor(cursor: string | undefined): ListCursor | undefined {
  ...
  try {
    ...
    assertText(value.id);
    if (encodeCursor(value as StoredPlan) !== cursor) invalid('Invalid transaction cursor.');
    return value;
  } catch {
