...
export function sponsorshipSchemas(): Record<string, Schema> {
  return {
    ...
    CreateSponsorship: object({ planId: ref("ResourceId"), stepIndexes: array(index, { minItems: 1, maxItems: RELAYR_LIMITS.maximumCalls, uniqueItems: true,
      description: "Omit to select all source steps, subject to the same limit. One independently executable call per chain; prerequisites must already be confirmed." }) }, ["planId"]),
    ...
    RelayrPayment: object({ chainId: chain, to: ref("Address"), data: ref("HexBytes"), value: ref("Uint256"), deadline: ref("Uint256") }),
    ...
    Sponsorship: object({ id: ref("ResourceId"), planId: ref("ResourceId"), planCommitment: ref("Hash"), commitment: ref("Hash"),
      ...
      quote: object({ bundleUuid: text, commitment: ref("Hash"), payments: array(ref("RelayrPayment"), { maxItems: RELAYR_MAINNET_CHAINS.length }), runtimeVerified: boolean,
