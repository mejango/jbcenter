...
ADD CONSTRAINT rest_wallet_deployment_dispatch_admission_v3 CHECK ((jsonb_typeof(admission)='object' AND octet_length(admission::text)<=4096
    ...
    AND (admission->>'expiresAt')::bigint<=(admission->>'observedAt')::bigint
      +CASE WHEN admission->>'version'='center-wallet-deployment-base-admission-v1' THEN 20000 ELSE 5000 END
