-- A passkey wallet created through Center is bound to its account by the consent its passkey already
-- gave (the enrollment possession proof, or the recovery proof): no setup document, no browser grant.
...
ALTER TABLE rest_smart_account_bindings ADD CONSTRAINT rest_smart_account_bindings_document_check CHECK (
  ...
  AND (
    ...
    OR (
      document->'authorization'->>'method' = 'center-wallet-passkey-creation-v1'
      AND (
        NOT (document->'authorization' ? 'setup')
        AND chain_id = 8453
        AND account_id = 'eip155:8453:' || wallet_address
        AND lower(document->>'ownerAddress') = wallet_address
        AND (document->'state'->>'chainId')::bigint = 8453
