-- A hosted-provider authority observation takes ~25 s from its observedAt, so a verified
-- snapshot stays ready for 120 s (was 30 s); walletAuthorityMaximumAgeMs matches.
  ...
  ADD CONSTRAINT rest_wallet_authority_snapshot_shape CHECK ((
    ...
    OR (jsonb_typeof(snapshot)='object' AND octet_length(snapshot::text)<=32768
      ...
      AND ((snapshot->>'readiness'='verified' AND snapshot->'bootstrapRequired'='false'::jsonb
          ...
          AND ready_until_ms<=(snapshot->'latestObservation'->>'observedAtMs')::bigint+120000
